...
def is_valid_trendyol_product_url(url: str) -> bool:
    try:
        parsed = urlparse(url.strip())
    except Exception:
        return False
    host = parsed.netloc.lower().replace("www.", "")
    return host in {"trendyol.com", "m.trendyol.com"} and bool(
        PRODUCT_PATH_RE.search(parsed.path or "")
    )


def is_generic_trendyol_title(name: str | None) -> bool:
    text = (name or "").strip().lower()
    if not text:
        return True
    blocked = (
        "online alışveriş sitesi",
        "türkiye'nin trend yolu",
        "turkiye'nin trend yolu",
        "trendyol - online",
        "trendyol.com",
    )
...
def cleanup_invalid_products() -> int:
    """Yanlışlıkla kaydedilmiş ana sayfa/kategori kayıtlarını güvenle temizler."""
    with get_conn() as conn:
        ...
        invalid_ids = [
            ...
            for row in rows
            if (
                not is_valid_trendyol_product_url(row["url"])
                or is_generic_trendyol_title(row["name"])
            )
        ]
        if not invalid_ids:
            return 0
        ...
        conn.execute(
            f"DELETE FROM snapshots WHERE product_id IN ({placeholders})",
            invalid_ids,
        )
        conn.execute(
        ...
        conn.commit()
        logger.info("Cleaned %s invalid product rows", len(invalid_ids))
...
def normalize_trendyol_url(url: str) -> str:
    ...
    clean = f"https://www.trendyol.com{parsed.path}".rstrip("/")
    if not is_valid_trendyol_product_url(clean):
        raise ValueError("Bağlantı bir Trendyol ürün sayfası değil.")
    return clean
...
def parse_public_product_page(html: str, url: str) -> dict[str, Any]:
    ...
    if not is_valid_trendyol_product_url(url):
        raise ValueError("Açılan sayfa gerçek bir Trendyol ürün sayfası değil.")
    if is_generic_trendyol_title(str(name)):
        raise ValueError("Trendyol ana sayfası ürün gibi algılandı; kayıt reddedildi.")
...
async def discover_all() -> dict[str, Any]:
    """
    Low-memory discovery for small Render instances:
    - only one discovery request can run at a time
    - one Chromium browser and one category page at a time
    - scans at most two categories per run
    - adds at most two products
    ...
    if DISCOVERY_LOCK.locked():
        return {
            "ok": False,
            "busy": True,
            "engine": "native_android_v8",
            "added": 0,
            "failed": 0,
            "candidates": 0,
            "categories": [],
            "notice": "Başka bir otomatik tarama zaten çalışıyor.",
        }
    ...
    async with DISCOVERY_LOCK:
        ...
        # Rotate through categories across runs without opening many pages at once.
        start_index = before % max(len(CATEGORY_SOURCES), 1)
        sources = [
            CATEGORY_SOURCES[start_index % len(CATEGORY_SOURCES)],
            CATEGORY_SOURCES[(start_index + 1) % len(CATEGORY_SOURCES)],
        ]
        ...
        async with async_playwright() as playwright:
            ...
            try:
                for source in sources:
                    if total_added >= 2 or existing_product_count() >= MAX_AUTO_PRODUCTS:
                        break

                    try:
                        candidates, page_info = await asyncio.wait_for(
                    ...
                    try:
                        data = await asyncio.wait_for(
                            fetch_public_product(fresh_candidate["url"]),
                            timeout=15,
                        )
                        data["category"] = source["name"]
                        data["rank_position"] = (
                            fresh_candidate.get("badge_rank")
                            or fresh_candidate.get("page_position")
                        )
                        data["source"] = "browser_category_discovery"
                        data["discovery_badge"] = fresh_candidate.get("badge_type")
                        save_product_and_snapshot(data)
                        row["added"] = 1
                        total_added += 1
                    except Exception as exc:
                        row["failed"] = 1
                        row["errors"].append(str(exc)[:180])
                        total_failed += 1
        ...
        return {
            ...
            "notice": (
                "Düşük bellek modu: kategoriler tek tek taranır ve "
                "her çalıştırmada en fazla iki yeni ürün eklenir."
            ),
        }
...
class NativeWatchRequest(BaseModel):
    url: str = Field(min_length=10)
...
async def lifespan(_: FastAPI):
    ...
    # v8: Render tarafındaki Trendyol taramaları devre dışı.
    # Keşif ve ürün gözlemleri Android telefondan gelir.
    yield
...
def health():
    return {
        ...
        "discovery_engine": "native_android_v8",
...
@app.post("/api/native-watch")
def native_watch(request: NativeWatchRequest):
    """Android WebView'in telefonda okuduğu gerçek ürün verisini kaydeder."""
    try:
        url = normalize_trendyol_url(request.url)
        name = request.name.strip()
        if is_generic_trendyol_title(name):
            raise ValueError("Genel Trendyol sayfası ürün olarak kaydedilemez.")

        data = {
            "platform": "Trendyol",
            "url": url,
            "name": name,
            "category": (request.category or "Genel").strip() or "Genel",
        ...
        return {
            ...
            "product_id": product_id,
            "product": data,
            "notice": "Telefon gözlemi kaydedildi.",
        }
    except Exception as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
...
async def discovery_now():
    return {
        ...
        "failed": 0,
        "candidates": 0,
        "notice": "v8 keşfi Android telefon uygulamasında çalışır.",
    }
...
<div class="app">
<header class="top">
  <div><div class="brand">Ürün<span>Radar</span></div><div class="sub">Trendyol Telefon Radar v8</div></div>
...
async function jsonFetch(path,options={}){
  ...
  try{
    ...
    if(e.name==="AbortError")throw new Error("Tarama 70 saniyeyi aştı ve durduruldu.");
    throw e;
  }finally{
    clearTimeout(timer);
  }
...
document.getElementById("discoverBtn").onclick=async()=>{
  ...
  try{
    ...
    if(r.busy){
      toast("Başka bir tarama zaten çalışıyor. Biraz sonra tekrar dene.");
      return;
    }
