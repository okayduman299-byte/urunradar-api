...
<section id="addPage" class="page hidden">
  <div class="page-head"><button class="back" data-route="home">‹</button><div><h1>Ürün Ekle</h1><p>Bir Trendyol ürün bağlantısını izlemeye al</p></div></div>
  <div class="form-card">
...
<nav class="nav">
  ...
  <button data-route="add">＋<span>Ürün Ekle</span></button>
</nav>
...
function render(){
  ...
  document.getElementById("topProducts").innerHTML=list.length?list.slice(0,4).map(card).join(""):`<div class="empty">Henüz ürün yok. “Ürün Ekle” bölümünden bir Trendyol bağlantısı ekle.</div>`;
