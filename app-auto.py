...
def extract_product_candidates(
    ...
    Sıra, sayfada görülen listedeki konumdur; gerçek satış sırası değildir.
    """
    soup = BeautifulSoup(html, "html.parser")
    seen: set[str] = set()
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if href.startswith("http"):
            ...
            if host not in {"trendyol.com", "m.trendyol.com"}:
                continue
            url = f"https://www.trendyol.com{parsed.path}".rstrip("/")
        else:
            path = href.split("?", 1)[0]
            if not path.startswith("/"):
                path = "/" + path
            url = f"https://www.trendyol.com{path}".rstrip("/")
...
async def fetch_search_candidates(keyword: str, limit: int = 20) -> list[dict[str, Any]]:
    ...
    async with httpx.AsyncClient(
        ...
        response.raise_for_status()
    return extract_product_candidates(response.text, limit=limit)


async def discover_keyword(keyword: str, max_new: int) -> dict[str, Any]:
    candidates = await fetch_search_candidates(keyword, limit=max(max_new * 6, 12))
    added = 0
    failed = 0
    checked = 0
...
async def discover_all() -> dict[str, Any]:
    ...
    return {
        ...
        "failed": total_failed,
        "keywords": results,
        "notice": "Ürünler kamuya açık pazar sinyallerinden otomatik keşfedildi.",
    }
...
document.getElementById("discoverBtn").onclick=async()=>{
  ...
  try{
    ...
    toast(`${r.added} yeni ürün otomatik eklendi.`);
