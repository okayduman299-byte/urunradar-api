...
async def fetch_category_candidates(
    ...
    """
    Public Trendyol kategori sayfalarının sayfalı sürümlerini tara.

    Bazı istemcilerde ilk kategori sayfası yalnızca kabuk HTML döndürebilir.
    Bu nedenle ?pi=2, ?pi=3 ve ?pi=4 sayfaları da kontrol edilir.
    """
    headers = {
        "User-Agent": USER_AGENT,
        "Accept-Language": "tr-TR,tr;q=0.9,en;q=0.7",
...
async def discover_category(
    ...
    for candidate in candidates:
        ...
        try:
            ...
            data["source"] = "category_discovery"
            ...
            logger.info("Category discovery skip %s: %s", url, exc)
...
async def discover_all() -> dict[str, Any]:
    ...
    for source in CATEGORY_SOURCES:
        ...
        try:
            result = await discover_category(
                source,
                DISCOVERY_LIMIT_PER_KEYWORD,
            )
        except Exception as exc:
            logger.warning(
                "Category discovery failed for %s: %s",
                source["name"],
                exc,
            )
            result = {
        ...
        total_candidates += int(result.get("candidates", 0))

    return {
        "ok": True,
        "engine": "category_pages_v4",
        "before": before,
        ...
        "categories": results,
        "notice": (
            "Ürünler kamuya açık Trendyol kategori sayfalarındaki ürün ve "
            "sıralama sinyallerinden keşfedildi."
        ),
    }
...
def health():
    return {
        ...
        "discovery_engine": "category_pages_v4",
...
<div class="app">
<header class="top">
  <div><div class="brand">Ürün<span>Radar</span></div><div class="sub">Trendyol Otomatik Radar v4</div></div>
...
document.getElementById("discoverBtn").onclick=async()=>{
  ...
  try{
    ...
    const diagnostic = firstAttempt
      ? `Sayfa ${firstAttempt.page}: HTTP ${firstAttempt.http_status || "?"}, ${firstAttempt.html_bytes || 0} bayt, ${firstAttempt.candidates || 0} aday.`
      : "";
    toast(
      r.added > 0
